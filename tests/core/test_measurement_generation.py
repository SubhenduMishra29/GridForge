...
def _channel(signal_type: MeasurementSignalType) -> MeasurementChannel:
    unit = "A" if signal_type is MeasurementSignalType.CURRENT else "V"
...
def _prepared_power_flow() -> PreparedPowerFlow:
    ...
    return PreparedPowerFlow(
        ...
        ybus=ybus,
        base_mva=100.0,
        bus_voltage_bases={"BUS-2": 11.0, "BUS-1": 11.0},
    )


def _current_base_a() -> float:
    return 100.0 * 1000.0 / (np.sqrt(3.0) * 11.0)
...
def test_line_current_uses_pu_to_base_to_ct_secondary_chain() -> None:
...
def test_current_conversion_requires_prepared_numerical_base() -> None:
...
def test_power_flow_voltage_uses_bus_id_and_physical_voltage_base() -> None:
    prepared = _prepared_power_flow()
...
def test_supported_short_circuit_fault_quantity_uses_pu_to_base_to_ct_chain() -> None:
...
def test_short_circuit_requires_prepared_numerical_base() -> None:
