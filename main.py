...
def _build_application_impl(resources: dict[str, object]) -> tuple[QApplication, MainWindow, PluginManager, WorkspaceController, UIUpdateBoundary, UILifecycle]:
    ...
    def deserialize_sld(data: dict) -> SLDDocument:
        ...
        if not isinstance(document, SLDDocument): raise TypeError("Persistent presentation must deserialize to SLDDocument")
        return document
    ...
    canvas_composer = CanvasComposer(); canvas_preparation = canvas_composer.prepare(controller=controller)
    ...
    canvas_plugin = canvas_entry.plugin; panels_plugin = panels_entry.plugin
    ...
    project_workspace_adapter = ProjectWorkspaceApplicationAdapter(application=gridforge_application, lifecycle=project_workspace_lifecycle); resources["project_workspace_adapter"] = project_workspace_adapter
    ...
    sld_controller = SLDController(projection_manager=sld_projection_manager, application=gridforge_application); sld_controller.register_document(sld_document); sld_controller.activate_document(sld_document.document_id)
    ...
    # Workspace realization is deliberately staged until all required presentation docks exist.
    ...
    element_list_projection = ElementListProjection(application=gridforge_application, panel=element_list_panel)
    project_hierarchy_projection = ProjectHierarchyProjection(adapter=project_workspace_adapter, panel=project_panel)
    validation_projection = ValidationProjection(application=gridforge_application, panel=messages_panel)
    study_projection = StudyProjection(application=gridforge_application, panel=study_cases_panel)
