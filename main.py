...
def _build_application_impl(resources: dict[str, object]) -> tuple[QApplication, MainWindow, PluginManager, WorkspaceController, UIUpdateBoundary, UILifecycle]:
    ...
    def deserialize_sld(data: dict) -> SLDDocument:
        ...
        if not isinstance(document, SLDDocument):
            raise TypeError("Persistent presentation must deserialize to an SLDDocument")
        return document
    ...
    project_workspace_adapter = ProjectWorkspaceApplicationAdapter(application=gridforge_application, lifecycle=project_workspace_lifecycle)
    ...
    sld_controller = SLDController(projection_manager=sld_projection_manager, application=gridforge_application)
    ...
    context = PluginContext(
        ...
        metadata={"sld_canvas_snapshot": sld_canvas_snapshot, "project_id": project_context.project_id, "project_workspace_adapter": project_workspace_adapter, "panel_presentation_bridge": panel_presentation_bridge},
    )
    ...
    canvas_composer.bind_selection_projection(composition=canvas_composition, properties_panel=properties_panel)
    ...
    ui_update_boundary = UIUpdateBoundary(event_bus=gridforge_application.event_bus, projection_coordinator=projection_coordinator)
    ui_update_boundary.subscribe()
    sld_update_coordinator.refresh(ProjectLoaded(metadata={"project_id": project_context.project_id, "operation": "initial"}))
    ...
    resources.update({"application": gridforge_application, "window": window, "ui_update_boundary": ui_update_boundary, "ui_lifecycle": ui_lifecycle})
    return app, window, plugin_manager, workspace_controller, ui_update_boundary, ui_lifecycle


def build_application(resources: dict[str, object] | None = None):
...
def main() -> int:
    app, window, plugin_manager, workspace_controller, ui_update_boundary, ui_lifecycle = build_application()
    window.show()
    return app.exec()
