...
class TransformerTool(ToolBase):
    ...
    The tool owns only transient placement interaction. Persistent
    transformer creation must eventually be performed by a
    Core-backed command through the established Application boundary.
    """
    ...
    def on_mouse_press(self, event: Any) -> bool:
        ...
        Persistent Core mutation is intentionally not performed
        because no confirmed CreateTransformer command exists yet.
        """
        self._ensure_active()
    ...
    def on_mouse_release(self, event: Any) -> bool:
        """
        Complete the placement interaction at the command boundary.

        Until a confirmed Core CreateTransformer command exists,
        persistent creation is rejected explicitly rather than
        bypassing the Application architecture.
        """
        self._ensure_active()
    ...
    def _snap_position(
        ...
        """Resolve an event position through the canonical SnapSystem API."""
    ...
    def _require_transformer_command_boundary() -> None:
        """
        Fail explicitly until the Core transformer-creation command
        is defined.

        This prevents accidental direct model mutation and avoids
        inventing a speculative command/factory API.
        """
        raise RuntimeError(
