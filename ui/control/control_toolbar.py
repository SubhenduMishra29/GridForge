...
class ControlToolbar(QWidget):
    def __init__(self, *, application: Any, on_add_rung, on_remove_rung, on_cancel_tool, parent: QWidget | None = None) -> None:
        super().__init__(parent)
        self._application = application
        layout = QHBoxLayout(self)
        layout.addWidget(QLabel("Control", self))
        add = QPushButton("Add Rung", self)
        add.clicked.connect(on_add_rung)
        layout.addWidget(add)
        remove = QPushButton("Remove Rung", self)
        remove.clicked.connect(on_remove_rung)
        layout.addWidget(remove)
        undo = QPushButton("Undo", self)
        undo.clicked.connect(application.undo)
        layout.addWidget(undo)
        redo = QPushButton("Redo", self)
        redo.clicked.connect(application.redo)
        layout.addWidget(redo)
        cancel = QPushButton("Cancel Tool", self)
        cancel.clicked.connect(on_cancel_tool)
