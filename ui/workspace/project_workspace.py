...
class ProjectWorkspaceLifecycle:
    ...
    def __init__(self, workspace_controller: WorkspaceController) -> None:
        if not isinstance(workspace_controller, WorkspaceController): raise TypeError("workspace_controller must be a WorkspaceController.")
        self._workspace_controller = workspace_controller; self._documents = DocumentManager(); self._views = ViewManager(); self._project: Project | None = None
    ...
    def state(self) -> ProjectWorkspaceState:
        document = self._documents.active_document; view = self._views.active_view
        return ProjectWorkspaceState(project=self._project, document=document, workspace_id=self._workspace_controller.active_workspace_id, view_id=view.view_id if view is not None else None)

    def new_project(self, project: Project, *, document: Document | None = None, document_type: str = "sld", document_name: str = "Untitled", workspace_id: str | None = None, activate_workspace: bool = True) -> ProjectWorkspaceState:
        ...
        self._clear_active_presentation(); self._project = project
        if document is None: document = Document(document_id=str(uuid4()), project_id=project.project_id, document_type=document_type, name=document_name)
        self._documents.register(document)
        try:
            if activate_workspace: self._activate_workspace(workspace_id)
        except BaseException:
            self._clear_failed_activation_state(); raise
        return self.state

    def open_project(self, project: Project, *, document: Document | None = None, workspace_id: str | None = None, activate_workspace: bool = True) -> ProjectWorkspaceState:
        if not isinstance(project, Project): raise TypeError("project must be a Project.")
        if document is not None and not isinstance(document, Document): raise TypeError("document must be a Document or None.")
        if document is not None and document.project_id not in (None, project.project_id): raise ValueError("document belongs to a different project.")
        self._clear_active_presentation(); self._project = project
        if document is not None: self._documents.register(document)
        try:
            if activate_workspace: self._activate_workspace(workspace_id)
        except BaseException:
            self._clear_failed_activation_state(); raise
    ...
    def close_project(self) -> ProjectWorkspaceState:
        self._clear_active_presentation(); return self.state
    ...
    def replace_document(self, document: Document) -> ProjectWorkspaceState:
        ...
        self._documents.register(document); return self.state
    ...
    def add_view(self, view: ViewRecord) -> ProjectWorkspaceState:
        ...
        self._views.register(view); return self.state

    def remove_document(self, document_id: str) -> Document:
        for view in self._views.views_for_document(document_id): self._views.unregister(view.view_id)
        return self._documents.unregister(document_id)

    def _activate_workspace(self, workspace_id: str | None) -> None:
        if workspace_id is None: self._workspace_controller.activate_default()
        else: self._workspace_controller.activate(workspace_id)

    def _clear_active_presentation(self) -> None:
        self._views.clear(); self._documents.clear(); self._workspace_controller.deactivate(); self._project = None

    def _clear_failed_activation_state(self) -> None:
        self._views.clear(); self._documents.clear(); self._project = None
