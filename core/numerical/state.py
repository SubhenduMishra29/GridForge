...
Numerical state is derived computational state. It is not physical Model
state, Network topology state, Study semantics, Solver algorithm state,
UI/SLD state, or persistence state.

BusState stores bus-level numerical quantities.
DynamicState stores a generic numerical state vector whose meaning is defined
by the corresponding physical/dynamic model or study formulation.
"""
...
class BusState:
    ...
    This container deliberately does not reference the physical Bus object
    and does not determine PQ/PV/Slack classification.
    """
    ...
    def _validate_real(value: Real, name: str) -> float:
        """Validate and normalize a real numerical value."""
    ...
    @property
    def p(self) -> float:
        """Active-power numerical quantity."""
        return self._p
    ...
    @property
    def q(self) -> float:
        """Reactive-power numerical quantity."""
        return self._q
...
class DynamicState:
    ...
    def _copy_values(values: Any) -> Any:
        """Return an independent copy of a supported value container."""
        if values is None:
            return None
    ...
    @property
    def values(self) -> Any:
        """Underlying numerical state values."""
        return self._values
    ...
    def validate(self) -> None:
        """Validate basic scalar or iterable numerical values.

        Detailed dimensional and equation-specific validation belongs to the
        corresponding numerical model or solver.
        """
        values = self.values
