...
class NewtonRaphsonSolver:
    ...
    def __init__(self, network, options):
        ...
        self.q_handler = QLimitHandler(network)
    ...
    def solve(self):
        ...
        jacobian_builder = JacobianBuilder(self.network)
        ...
        for iteration in range(self.options.max_iterations):
            ...
            mismatch_solver = PowerMismatch(self.network, Ybus)
            mismatch = mismatch_solver.compute()

            error = np.max(np.abs(mismatch))
            self.history.append(error)

            if self.options.verbose:
                print(f"Iteration {iteration+1}: Mismatch={error:.6e}")
            ...
            try:
                dx = self.linear_solver.solve(J, mismatch)
    ...
    def _update_states(self, dx):

        buses = self.network.buses

        # ---- Update voltage angles (Δθ)
        angle_index = 0

        for bus in buses:
            if not bus.is_slack():
                bus.theta += self.options.damping * dx[angle_index]
                angle_index += 1

        # ---- Update voltage magnitudes (ΔV)
        voltage_index = angle_index

        for bus in buses:
            if bus.is_pq():
                bus.V += self.options.damping * dx[voltage_index]
                voltage_index += 1
    ...
    def _voltage_result(self):
        ...
        return {
            "Vm": np.array([bus.V for bus in self.network.buses]),
            "Va": np.array([bus.theta for bus in self.network.buses])
        }
