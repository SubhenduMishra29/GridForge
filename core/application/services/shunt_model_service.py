# ============================================================
# File: core/application/services/shunt_model_service.py
# GridForge V2 — Shunt Model Service
...
class ShuntModelService(ModelServiceSupport):
    ...
    def create_shunt(self, *, shunt_id: str, name: str = "", endpoint: Bus | Terminal | None = None, g_pu: float = 0.0, b_pu: float = 0.0, in_service: bool = True, transaction: Transaction) -> ApplicationResult[Shunt]:
        ...
        self._network.add_shunt(shunt); transaction.record_undo(lambda shunt=shunt: self._network.remove_shunt(shunt)); return self._success(shunt, "shunt", shunt_id, f"Shunt created: {shunt_id}")
    ...
    def create_capacitor(self, *, capacitor_id: str, name: str = "", endpoint: Bus | Terminal | None = None, reactive_power_injection_mvar: float = 0.0, in_service: bool = True, transaction: Transaction) -> ApplicationResult[Capacitor]:
        ...
        self._network.add_capacitor(capacitor); transaction.record_undo(lambda capacitor=capacitor: self._network.remove_capacitor(capacitor)); return self._success(capacitor, "capacitor", capacitor_id, f"Capacitor created: {capacitor_id}")
    ...
    def update_capacitor(self, *, capacitor_id: str, name: str | None = None, endpoint: Bus | Terminal | None = None, reactive_power_injection_mvar: float | None = None, in_service: bool | None = None, transaction: Transaction) -> ApplicationResult[Capacitor]:
        self._require_transaction(transaction); self._require_id(capacitor_id, "capacitor_id"); capacitor = self._get_required("capacitor", capacitor_id, "Capacitor"); self._require_type(capacitor, Capacitor, capacitor_id, "Capacitor")
        if endpoint is not None: self._validate_endpoint(endpoint, "endpoint")
        if all(v is None for v in (name, endpoint, reactive_power_injection_mvar, in_service)): raise DomainError(code="NO_CAPACITOR_UPDATE", message="At least one mutable Capacitor property must be specified.", details={"capacitor_id": capacitor_id})
        old_endpoint = capacitor.endpoint; old = {"name": capacitor.name, "reactive_power_injection_mvar": capacitor.reactive_power_injection_mvar, "in_service": capacitor.in_service}
    ...
    def delete_capacitor(self, *, capacitor_id: str, transaction: Transaction) -> ApplicationResult[Capacitor]:
        self._require_transaction(transaction); self._require_id(capacitor_id, "capacitor_id"); capacitor = self._get_required("capacitor", capacitor_id, "Capacitor"); self._require_type(capacitor, Capacitor, capacitor_id, "Capacitor"); self._network.remove_capacitor(capacitor); transaction.record_undo(lambda capacitor=capacitor: self._network.add_capacitor(capacitor)); return self._success(capacitor, "capacitor", capacitor_id, f"Capacitor deleted: {capacitor_id}")

    def put_capacitor_in_service(self, *, capacitor_id: str, transaction: Transaction) -> ApplicationResult[Capacitor]: return self._set_capacitor_service(capacitor_id=capacitor_id, in_service=True, transaction=transaction)
    def take_capacitor_out_of_service(self, *, capacitor_id: str, transaction: Transaction) -> ApplicationResult[Capacitor]: return self._set_capacitor_service(capacitor_id=capacitor_id, in_service=False, transaction=transaction)

    def _set_capacitor_service(self, *, capacitor_id: str, in_service: bool, transaction: Transaction) -> ApplicationResult[Capacitor]:
        self._require_transaction(transaction); self._require_id(capacitor_id, "capacitor_id"); capacitor = self._get_required("capacitor", capacitor_id, "Capacitor"); self._require_type(capacitor, Capacitor, capacitor_id, "Capacitor"); old = capacitor.in_service; capacitor.in_service = in_service; transaction.record_undo(lambda capacitor=capacitor, old=old: setattr(capacitor, "in_service", old)); return self._success(capacitor, "capacitor", capacitor_id, f"Capacitor {'put in service' if in_service else 'taken out of service'}: {capacitor_id}")
