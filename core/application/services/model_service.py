...
class ModelService:
    ...
    def update_bus(
        ...
        *,
        ...
        """Mutate an existing Core Bus through its authoritative contract."""
        ...
        if nominal_voltage_kv is not None:
            bus.nominal_voltage_kv = bus._validate_non_negative(
                nominal_voltage_kv,
                "nominal_voltage_kv",
            )

        if frequency_hz is not None:
            bus.frequency_hz = bus._validate_positive(
                frequency_hz,
                "frequency_hz",
            )
        ...
        if voltage_pu is not None or angle_deg is not None:
            bus.set_voltage(
                bus.voltage_pu if voltage_pu is None else voltage_pu,
                bus.angle_deg if angle_deg is None else angle_deg,
            )
    ...
    def create_grid(self, *, grid_id: str, endpoint: Bus | Terminal | None = None, name: str | None = None, nominal_voltage_kv: float = 0.0, frequency_hz: float = 50.0, voltage_pu: float = 1.0, angle_deg: float = 0.0, p_mw: float = 0.0, q_mvar: float = 0.0, short_circuit_mva: float = 0.0, x_over_r: float = 0.0, z1_pu: complex = 0j, z2_pu: complex = 0j, z0_pu: complex = 0j, in_service: bool = True, grounded: bool = True, transaction: Transaction) -> ApplicationResult[Grid]:
        ...
        self._network.add_grid(grid); transaction.record_undo(lambda grid=grid: self._network.remove_grid(grid))
        return self._success(grid, "grid", grid_id, f"Grid created: {grid_id}")
    ...
    def update_grid(self, *, grid_id: str, name: str | None = None, nominal_voltage_kv: float | None = None, frequency_hz: float | None = None, voltage_pu: float | None = None, angle_deg: float | None = None, p_mw: float | None = None, q_mvar: float | None = None, short_circuit_mva: float | None = None, x_over_r: float | None = None, z1_pu: complex | None = None, z2_pu: complex | None = None, z0_pu: complex | None = None, in_service: bool | None = None, grounded: bool | None = None, transaction: Transaction) -> ApplicationResult[Grid]:
        self._require_transaction(transaction); self._require_id(grid_id, "grid_id")
        grid = self._get_required("grid", grid_id, "Grid"); self._require_type(grid, Grid, grid_id, "Grid")
    ...
    def create_generator(self, *, generator_id: str, endpoint: Bus | Terminal | None = None, p: float = 0.0, q: float = 0.0, V_setpoint: float = 1.0, q_limits: tuple[float, float] = (-float("inf"), float("inf")), name: str = "", in_service: bool = True, transaction: Transaction) -> ApplicationResult[Generator]:
        ...
        if endpoint is not None: self._validate_endpoint(endpoint, "endpoint")
        self._ensure_not_exists("generator", generator_id, "Generator")
        generator = Generator(id=generator_id, endpoint=endpoint, p=p, q=q, V_setpoint=V_setpoint, q_limits=q_limits, name=name, in_service=in_service)
        self._network.add_generator(generator); transaction.record_undo(lambda generator=generator: self._network.remove_generator(generator)); return self._success(generator, "generator", generator_id, f"Generator created: {generator_id}")
    ...
    def update_generator(self, *, generator_id: str, name: str | None = None, p: float | None = None, q: float | None = None, V_setpoint: float | None = None, q_limits: tuple[float, float] | None = None, in_service: bool | None = None, transaction: Transaction) -> ApplicationResult[Generator]:
        ...
        def restore():
            generator.name=old["name"]; generator.set_power(old["p"],old["q"]); generator.set_voltage_setpoint(old["V_setpoint"]); generator.set_q_limits(old["q_limits"][0],old["q_limits"][1]); generator.put_in_service() if old["in_service"] else generator.take_out_of_service()
    ...
    # ============================================================
    # LINE / TRANSFORMER / BRANCH / CABLE / SWITCH / DISCONNECTOR / FUSE
    # ============================================================
    ...
    def update_branch(self, *, branch_id: str, name: str | None = None, r: float | None = None, x: float | None = None, b: float | None = None, rate_mva: float | None = None, tap: float | None = None, shift: float | None = None, in_service: bool | None = None, transaction: Transaction) -> ApplicationResult[Branch]:
        ...
        def restore():
            branch.name=old["name"];branch.r=old["r"];branch.x=old["x"];branch.b=old["b"];branch.rate_mva=old["rate_mva"];branch.tap=old["tap"];branch.shift=old["shift"];branch.set_in_service(old["in_service"])
    ...
    def update_switch(self, *, switch_id: str, name: str | None = None, closed: bool | None = None, in_service: bool | None = None, normally_closed: bool | None = None, rated_voltage_kv: float | None = None, rated_current_a: float | None = None, transaction: Transaction) -> ApplicationResult[Switch]:
        ...
        def restore():
            switch.name=old["name"];switch.set_closed(old["closed"]);switch.set_in_service(old["in_service"]);switch.set_normally_closed(old["normally_closed"]);switch.rated_voltage_kv=old["rated_voltage_kv"];switch.rated_current_a=old["rated_current_a"]
    ...
    def update_disconnector(self, *, disconnector_id: str, voltage_kv: float | None = None, rated_current_a: float | None = None, operating_time: float | None = None, closed: bool | None = None, in_service: bool | None = None, name: str | None = None, transaction: Transaction) -> ApplicationResult[Disconnector]:
        ...
        def restore():
            disconnector.voltage_kv=old["voltage_kv"];disconnector.rated_current_a=old["rated_current_a"];disconnector.operating_time=old["operating_time"];disconnector.set_closed(old["closed"]);disconnector.set_in_service(old["in_service"]);disconnector.name=old["name"]
    ...
    def update_fuse(self, *, fuse_id: str, name: str | None = None, rated_current_a: float | None = None, rated_voltage_v: float | None = None, interrupting_rating_ka: float | None = None, in_service: bool | None = None, blown: bool | None = None, transaction: Transaction) -> ApplicationResult[Fuse]:
        ...
        def restore():
            fuse.name=old["name"];fuse.rated_current_a=old["rated_current_a"];fuse.rated_voltage_v=old["rated_voltage_v"];fuse.interrupting_rating_ka=old["interrupting_rating_ka"];fuse.set_in_service(old["in_service"]);fuse.blow() if old["blown"] else fuse.reset()
    ...
    # ============================================================
    # COMMON LOOKUP / VALIDATION
    # ============================================================

    def _get_required(self, element_type: str, object_id: str, display_type: str) -> Any:
        try:value=self._network.get_by_id(element_type,object_id)
        except KeyError as exc:raise ResourceError(code=f"{element_type.upper()}_NOT_FOUND",message=f"{display_type} not found: {object_id}",details={"object_type":element_type,"object_id":object_id}) from exc
        if value is None:raise ResourceError(code=f"{element_type.upper()}_NOT_FOUND",message=f"{display_type} not found: {object_id}",details={"object_type":element_type,"object_id":object_id})
        return value
    def _ensure_not_exists(self, element_type: str, object_id: str, display_type: str) -> None:
        try:value=self._network.get_by_id(element_type,object_id)
        except KeyError:return
        if value is not None:raise DomainError(code=f"{element_type.upper()}_ALREADY_EXISTS",message=f"{display_type} already exists: {object_id}",details={"object_type":element_type,"object_id":object_id})
    @staticmethod
    def _require_type(value: Any, expected_type: type, object_id: str, display_type: str) -> None:
        if not isinstance(value,expected_type):raise DomainError(code=f"INVALID_{display_type.upper()}_REFERENCE",message=f"Object {object_id!r} is not a {display_type}.",details={"object_id":object_id,"object_type":type(value).__name__})
    @staticmethod
    def _validate_endpoint(endpoint: object, parameter_name: str) -> None:
        if not isinstance(endpoint,(Bus,Terminal)):raise DomainError(code="INVALID_ENDPOINT",message=f"{parameter_name} must be a Bus or Terminal.",details={"parameter":parameter_name,"object_type":type(endpoint).__name__})
    @staticmethod
    def _require_distinct_endpoints(endpoint_from: object, endpoint_to: object, code: str, display_type: str, object_id: str) -> None:
        if endpoint_from is endpoint_to:raise DomainError(code=code,message=f"{display_type} endpoints must be different.",details={"object_id":object_id})
    @staticmethod
    ...
    def _require_id(value: str, parameter_name: str) -> None:
        if not isinstance(value,str):raise TypeError(f"{parameter_name} must be str.")
        if not value.strip():raise ValueError(f"{parameter_name} must not be empty.")
    @staticmethod
    def _success(value: Any, object_type: str, object_id: str, message: str) -> ApplicationResult:
        return ApplicationResult.success_result(value=value,message=message,metadata={"object_type":object_type,"object_id":object_id})
