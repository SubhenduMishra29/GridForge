...
class ControlExecutionService:
    ...
    def execute(self, evaluation: ControlEvaluationResult) -> ControlExecutionResult:
        """Dispatch valid decisions in their supplied deterministic order.

        Invalid decisions and separately reported blocked actions are never
        dispatched. A failed Application command is recorded and execution
        continues with the next decision so one failed action cannot silently
        suppress later deterministic control intents.
        """
