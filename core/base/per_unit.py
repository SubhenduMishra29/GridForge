...
class PerUnitSystem:
    ...
    def __init__(self, base_mva: float) -> None:
        ...
        if not math.isfinite(base_mva):
            raise ValueError("Base MVA must be finite.")
        if base_mva <= 0.0:
    ...
    def _validate_voltage_base(kv: float) -> float:
        ...
        if not math.isfinite(kv):
            raise ValueError("Voltage base must be finite.")
        if kv <= 0.0:
    ...
    def z_base(self, kv: float) -> float:
        """Calculate base impedance: Z_base = kV_base² / MVA_base."""
    ...
    def y_base(self, kv: float) -> float:
        """Calculate base admittance: Y_base = 1 / Z_base."""
    ...
    def i_base(self, kv: float) -> float:
        ...
        kv = self._validate_voltage_base(kv)
        return self.base_mva / (math.sqrt(3.0) * kv)
    ...
    def to_pu_power(self, p_mw: float, q_mvar: float = 0.0) -> complex:
        ...
        return complex(float(p_mw), float(q_mvar)) / self.base_mva

    # -----------------------------------------------------------------

    def from_pu_power(self, s_pu: complex) -> Tuple[float, float]:
    ...
    def convert_impedance_base(
        ...
        Zpu_new = Zpu_old × (MVA_new / MVA_old)
                         × (kV_old / kV_new)²
        """
        old_mva = self._validate_mva_base(old_mva, "Old MVA base")
    ...
    def convert_admittance_base(
        ...
        Ypu_new = Ypu_old × (MVA_old / MVA_new)
                         × (kV_new / kV_old)²
        """
        old_mva = self._validate_mva_base(old_mva, "Old MVA base")
        old_kv = self._validate_voltage_base(old_kv)
    ...
    def z_base_vector(self, kv_array: np.ndarray) -> np.ndarray:
        ...
        kv_array = np.asarray(kv_array, dtype=float)
        if np.any(~np.isfinite(kv_array)):
            raise ValueError("Voltage-base array contains non-finite values.")
        if np.any(kv_array <= 0.0):
            raise ValueError("Voltage-base array must contain only positive values.")
    ...
    def to_pu_impedance_vector(
        ...
        if z_array.shape != kv_array.shape:
            raise ValueError("z_array and kv_array must have identical shapes.")
    ...
    def __repr__(self) -> str:
        ...
        return f"<PerUnitSystem base_mva={self.base_mva:.6f}>"
